    pipeline {
        ...
        stages {
                ...
                steps {
                    script {
                        withCredentials([usernamePassword(credentialsId: 'AzureServicePrincipal',
                                passwordVariable: 'AZURE_CLIENT_SECRET', usernameVariable: 'AZURE_CLIENT_ID')]) {
                            sh 'az login --service-principal -u $AZURE_CLIENT_ID -p $AZURE_CLIENT_SECRET -t ' +
                                    '$AZURE_TENANT_ID'
            ...
            stage('Terraform Apply') {
                steps {
                    script {
                        withCredentials([usernamePassword(credentialsId: 'AZURE_USER',
                                passwordVariable: 'AZURE_CLIENT_SECRET', usernameVariable: 'AZURE_CLIENT_ID')]) {
            ...
            stage('Build image') {
                steps {
                    withCredentials([usernamePassword(credentialsId: 'AzureServicePrincipal',
                        ...
                        sh 'az acr build --image $REPO/$IMAGE_NAME:$TAG --registry $CONTAINER_REGISTRY --file ' +
                                'Dockerfile . '
                    }
                }
            }
